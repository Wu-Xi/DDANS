...
def test(model, user_dict, train_sp_mat, n_params, valid_pre, test_pre, item_group_idx=None, mode='test'):
    ...
    train_sp_mat = train_sp_mat['train_sp_mat']
    if mode == 'test':
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        # swap 
        swap_idx = uid2swap_idx[user_list]      # [batch_size, xxx ]
        rev_swap_idx = uid2rev_swap_idx[user_list]      # swap_idx的倒序

        swap_row = torch.cat([torch.full_like(swap, i) for i, swap in enumerate(swap_idx)])     # 一维列表[0, ······, 2047]，共有2048*每个用户的positive item数量
        swap_col_after = torch.cat(list(swap_idx))      # 二维列表[batch_size, xxx]，共有2048*每个用户的positive item数量
    ...
    mean_hit = hit_(pos_idx_matrix, pos_len_list).mean(axis=0)  
...
def hit_(pos_index, pos_len):
    ...
    return (np.cumsum(pos_index, axis=1) > 0).astype(int)
